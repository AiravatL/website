...
export default function DeleteAccountFlow() {
  ...
  return (
    <div className="bg-slate-900 min-h-screen">
      <section className="pt-32 pb-16 bg-gradient-to-br from-slate-900 via-slate-800 to-red-900/20">
        <div className="max-w-4xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center">
            <div className="flex items-center justify-center space-x-3 mb-6">
              <AlertTriangle className="text-red-400" size={48} />
              <h1 className="text-5xl md:text-6xl font-light text-white">
                Delete <span className="text-red-400 font-medium">Account</span>
              </h1>
            </div>
          ...
          <div className="flex items-center justify-center space-x-4">
            {[1, 2, 3].map((n) => (
              <div key={n} className="flex items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold ${step >= n ? "bg-red-400 text-white" : "bg-slate-700 text-slate-400"}`}>
                  {n}
                </div>
                {n < 3 && <div className={`w-16 h-0.5 mx-2 ${step > n ? "bg-red-400" : "bg-slate-700"}`} />}
              </div>
            ))}
          </div>
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-6">
          {step === 1 && (
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              <div className="text-center mb-12">
                <h2 className="text-3xl font-semibold text-white mb-4">What happens when you delete your account?</h2>
                <p className="text-slate-400 max-w-2xl mx-auto">Please review the consequences of account deletion. This action cannot be undone.</p>
              </div>
              ...
              <div className="bg-red-400/10 border border-red-400/30 rounded-3xl p-8 mb-8">
          ...
          {step === 2 && (
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              <div className="text-center mb-12">
                <h2 className="text-3xl font-semibold text-white mb-4">Help us improve</h2>
                <p className="text-slate-400 max-w-2xl mx-auto">Your feedback is valuable to us. Please let us know why you&rsquo;re leaving so we can improve our services.</p>
              </div>

              <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8 mb-8">
                <h3 className="text-xl font-semibold text-white mb-6">What&rsquo;s your primary reason for leaving?</h3>
                <div className="space-y-3">
                  {deleteReasons.map((r) => (
                    <label key={r} className="flex items-center space-x-3 cursor-pointer">
                      <input type="radio" name="reason" value={r} checked={reason === r} onChange={(e) => setReason(e.target.value)} className="w-4 h-4 text-red-400 bg-slate-700 border-slate-600 focus:ring-red-400" />
              ...
              <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8">
                <h3 className="text-xl font-semibold text-white mb-4">Additional feedback (optional)</h3>
                <textarea
                  placeholder="Tell us more about your experience or how we could have served you better..."
                  className="w-full bg-slate-700 text-white p-4 rounded-xl border border-slate-600 focus:border-red-400 focus:outline-none resize-none"
                  rows={4}
                />
              </div>
            </motion.div>
          ...
          {step === 3 && (
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              <div className="text-center mb-12">
                <h2 className="text-3xl font-semibold text-white mb-4">Final confirmation</h2>
                <p className="text-slate-400 max-w-2xl mx-auto">To confirm account deletion, please type &ldquo;DELETE MY ACCOUNT&rdquo; in the field below.</p>
              </div>

              <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8 mb-8">
                <label className="block text-white font-semibold mb-4">Type &ldquo;DELETE MY ACCOUNT&rdquo; to confirm:</label>
                <input
                  type="text"
                  value={confirmText}
                  onChange={(e) => setConfirmText(e.target.value)}
                  placeholder="DELETE MY ACCOUNT"
                  className="w-full bg-slate-700 text-white p-4 rounded-xl border border-slate-600 focus:border-red-400 focus:outline-none"
                />
                {isConfirmed && (
              ...
              <div className="bg-red-400/10 border border-red-400/30 rounded-3xl p-8">
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-semibold text-white mb-4">Last chance to reconsider</h3>
          ...
          <div className="flex justify-between mt-12">
            ...
            <div className="flex space-x-4">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="border border-slate-600 text-white px-8 py-4 rounded-full text-lg font-medium hover:bg-slate-800 transition-colors">
                Cancel
              </motion.button>
              ...
              {step < 3 ? (
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className={`px-8 py-4 rounded-full text-lg font-medium transition-colors ${step === 2 && !reason ? "bg-slate-700 text-slate-400 cursor-not-allowed" : "bg-red-500 text-white hover:bg-red-400"}`}
                >
                  Continue
                </motion.button>
              ) : (
                <motion.button
                  ...
                  className={`px-8 py-4 rounded-full text-lg font-medium transition-colors ${!isConfirmed ? "bg-slate-700 text-slate-400 cursor-not-allowed" : "bg-red-500 text-white hover:bg-red-400"}`}
                >
                  Delete Account
                </motion.button>
              )}
      ...
      <section className="py-16 bg-slate-800/50">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8">
